...
exports.getList = (type, keyword, classId, pageindex, pagesize) => {
  return new Promise((resolve, reject) => {
    ...
    if (type === 0) {
      ...
      sql += " and feature.brief like '%" + keyword + "%' ESCAPE '\' order by id desc limit ?,?"
...
exports.getHotList=()=>{
  return new Promise((resolve, reject) => {
    ...
    let sql = "select * from (select * from (select * from (select id,title,likes,clicks from feature order by id desc) AS temp limit 0,20) AS temp1 order by clicks desc) AS temp2 limit 1,8";
